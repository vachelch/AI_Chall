...
def build_vocab(train_dir, label_dir, vocab_dir, vocab_size=5000):
    """根据训练集构建词汇表，存储"""
...
def process_file(filename, labelname, word_to_id, cat_to_id, max_length=600, train = False):
    ...
    """ obtain weight loss """
    loss_weights = [0 for c in categories]
    for label in labels:
        idx = cat_to_id[label]
        loss_weights[idx] += 1
    mx = max(loss_weights)
    loss_weights = [mx / (cnt + 0.1) for cnt in loss_weights]
    ...
    return x_pad, y_pad, loss_weights
